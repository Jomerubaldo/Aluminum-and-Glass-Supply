import { useState, useEffect } from 'react';
import { Button } from '@/components/ui/button';
import {
  Dialog,
  DialogClose,
  DialogContent,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from '@/components/ui/dialog';
import Swal from 'sweetalert2';
import { Field, FieldGroup } from '@/components/ui/field';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { SquarePen } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/components/ui/select';
import { RadioGroup, RadioGroupItem } from '@/components/ui/radio-group';
import { updateOrder } from '@/services/api';

const EditDialog = ({ order, customers = [], products = [], onSuccess }) => {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [requestType, setRequestType] = useState('AUTO');
  const [selectedProduct, setSelectedProduct] = useState(null);
  const [form, setForm] = useState({
    customerID: '',
    productID: '',
    quantity: 1,
    height: '',
    width: '',
    specification: '',
  });

  useEffect(() => {
    if (order) {
      const product = products.find((p) => p.productName === order.productName);
      const customer = customers.find(
        (c) => c.customerName === order.customerName
      );
      setForm({
        customerID: customer ? String(customer.customerID) : '',
        productID: product ? String(product.productID) : '',
        quantity: order.quantity || 1,
        height: order.height || '',
        width: order.width || '',
        specification: order.specification || '',
      });
      setSelectedProduct(product || null);
      setRequestType(order.requestType || 'AUTO');
    }
  }, [order, customers, products]);

  const total = selectedProduct ? selectedProduct.price * form.quantity : 0;

  const handleSubmit = async (e) => {
    e.preventDefault();

    if (!form.customerID || !form.productID) {
      return Swal.fire({
        icon: 'warning',
        title: 'Missing Information',
        text: 'Customer and product are required.',
        confirmButtonColor: '#f59e0b',
      });
    }

    if (!form.quantity || form.quantity < 1) {
      return Swal.fire({
        icon: 'warning',
        title: 'Invalid Quantity',
        text: 'Quantity must be at least 1.',
        confirmButtonColor: '#f59e0b',
      });
    }

    try {
      setLoading(true);

      await updateOrder(order.orderID, {
        ...form,
        requestType,
        price: selectedProduct?.price || order?.price || 0,
      });

      setOpen(false);

      Swal.fire({
        icon: 'success',
        title: 'Order Updated!',
        text: 'Order has been updated successfully.',
        showConfirmButton: false,
        timer: 2000,
        timerProgressBar: true,
      });

      onSuccess();
    } catch (err) {
      Swal.fire({
        icon: 'error',
        title: 'Update Failed',
        text: err.response?.data?.error || 'Error updating order',
        confirmButtonColor: '#ef4444',
      });
    } finally {
      setLoading(false);
    }
  };

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger
        render={
          <Button variant="outline" className="cursor-pointer rounded-md">
            <SquarePen />
          </Button>
        }
      />
      <DialogContent className="w-full max-w-[90vw] sm:max-w-md">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle className="text-base sm:text-lg">
              Edit Order
            </DialogTitle>
            <p className="text-xs text-muted-foreground mt-0.5">
              Update order details below
            </p>
          </DialogHeader>

          <div className="mt-4 max-h-[55vh] overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-gray-400 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent space-y-4 border-t pt-4">
            <FieldGroup className="space-y-4">
              <Field className="space-y-1">
                <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
                  Customer
                </Label>
                <Select
                  value={form.customerID}
                  onValueChange={(val) => setForm({ ...form, customerID: val })}
                >
                  <SelectTrigger className="w-full text-sm">
                    <SelectValue placeholder="Select customer" />
                  </SelectTrigger>
                  <SelectContent>
                    {customers.map((c) => (
                      <SelectItem
                        key={c.customerID}
                        value={String(c.customerID)}
                      >
                        {c.customerName}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </Field>

              <div className="grid grid-cols-2 gap-4">
                <Field className="space-y-1">
                  <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
                    Product
                  </Label>
                  <Select
                    value={form.productID}
                    onValueChange={(val) => {
                      const p = products.find(
                        (p) => String(p.productID) === val
                      );
                      setSelectedProduct(p || null);
                      setForm({ ...form, productID: val });
                    }}
                  >
                    <SelectTrigger className="w-full text-sm">
                      <SelectValue placeholder="Select product" />
                    </SelectTrigger>
                    <SelectContent>
                      {products.map((p) => (
                        <SelectItem
                          key={p.productID}
                          value={String(p.productID)}
                        >
                          {p.productName}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </Field>

                <Field className="space-y-1">
                  <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
                    Quantity
                  </Label>
                  <Input
                    type="number"
                    min={1}
                    className="text-sm"
                    value={form.quantity}
                    onChange={(e) =>
                      setForm({ ...form, quantity: Number(e.target.value) })
                    }
                  />
                </Field>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <Field className="space-y-1">
                  <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
                    Price
                  </Label>
                  <Input
                    type="text"
                    className="bg-muted text-sm font-medium cursor-not-allowed"
                    readOnly
                    value={
                      selectedProduct
                        ? `₱${selectedProduct.price.toLocaleString()}`
                        : order?.price
                          ? `₱${Number(order.price).toLocaleString()}`
                          : '—'
                    }
                  />
                </Field>

                <Field className="space-y-1">
                  <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
                    Measurement (W × H)
                  </Label>
                  <div className="flex gap-2">
                    <Input
                      type="number"
                      min={0}
                      placeholder="W"
                      className="text-sm text-center"
                      value={form.width}
                      onChange={(e) =>
                        setForm({ ...form, width: e.target.value })
                      }
                    />
                    <span className="self-center text-xs text-muted-foreground">
                      ×
                    </span>
                    <Input
                      type="number"
                      min={0}
                      placeholder="H"
                      className="text-sm text-center"
                      value={form.height}
                      onChange={(e) =>
                        setForm({ ...form, height: e.target.value })
                      }
                    />
                  </div>
                </Field>
              </div>

              <Field className="space-y-1">
                <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
                  Specs
                </Label>
                <Input
                  type="text"
                  className="text-sm"
                  placeholder="Enter specifications..."
                  value={form.specification}
                  onChange={(e) =>
                    setForm({ ...form, specification: e.target.value })
                  }
                />
              </Field>

              <Field className="space-y-2 border-t pt-3">
                <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
                  Material Requirements
                </Label>
                <RadioGroup
                  value={requestType}
                  onValueChange={setRequestType}
                  className="flex gap-6 pt-1"
                >
                  {['AUTO', 'MANUAL'].map((type) => (
                    <div
                      key={type}
                      className="flex items-center gap-2 cursor-pointer"
                    >
                      <RadioGroupItem value={type} id={`edit-${type}`} />
                      <Label
                        htmlFor={`edit-${type}`}
                        className="cursor-pointer text-sm font-medium"
                      >
                        {type === 'AUTO' ? 'AUTO' : 'MANUAL'}
                      </Label>
                    </div>
                  ))}
                </RadioGroup>

                <div
                  className={`text-xs p-2.5 rounded-md border mt-2 ${
                    requestType === 'AUTO'
                      ? 'bg-blue-500/5 text-blue-600 border-blue-500/10'
                      : 'bg-yellow-500/5 text-yellow-600 border-yellow-500/10'
                  }`}
                >
                  {requestType === 'AUTO'
                    ? `System will auto-generate materials needed for ${selectedProduct?.productName || order?.productName || 'the product'}.`
                    : 'Manual mode enabled. No suggestions will be populated.'}
                </div>
              </Field>

              <div className="flex items-center justify-between pt-4 border-t mt-2">
                <p className="text-sm font-medium">Total</p>
                <p className="text-green-600 font-medium text-base sm:text-lg">
                  ₱{total.toLocaleString()}
                </p>
              </div>
            </FieldGroup>
          </div>

          <DialogFooter className="mt-4 pt-4 border-t flex flex-col sm:flex-row gap-2">
            <Button
              type="submit"
              disabled={loading}
              className="cursor-pointer w-full sm:w-auto bg-foreground text-white transition-colors"
            >
              {loading ? 'Saving...' : 'Save Changes'}
            </Button>
            <DialogClose
              render={
                <Button
                  type="button"
                  variant="outline"
                  className="cursor-pointer w-full sm:w-auto"
                >
                  Cancel
                </Button>
              }
            />
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
};

export default EditDialog;
